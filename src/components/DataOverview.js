...
import { makeBullets } from "./ProjectOverview";
import { useUIStore } from "./store/store";
//
// Project Data Overview Functional component
//

export default function DataOverview({ selected, flexHeight }) {
  //
  // State
  //
  ...
  if (model) {
    return (
      ...
      >
        <div className="data">
          <h5 style={{ padding: 10 }}>{selected.id}</h5>
          <Accordion alwaysOpen="true">
            <Accordion.Item eventKey="metadata">
              <Accordion.Header>Model Info</Accordion.Header>
              <Accordion.Body>
                <ModelInfo
                  type={model.type}
                  users={model.users}
  ...
  return (
    <div className="data-overview">
      <div className="data" style={{ fontSize: 20, padding: 10 }}>
...
function ModelInfo({ users, type, requirements, scenarioMappings, other }) {
  return (
    <div className="metadata">
      ...
      <ScenarioMapping data={scenarioMappings} />
    </div>
  );
}
