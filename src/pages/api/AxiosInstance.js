...
pipes.interceptors.request.use(
  (config) => {
...
pipes.interceptors.response.use(
  (response) => response,
  (error) => {
    if (error.response) {
      ...
      switch (status) {
        case 400:
          throw new Error(data.message || "Bad Request: Invalid data provided.");
        case 401:
          throw new Error("Unauthorized: Please check your access token.");
        case 403:
          throw new Error("Forbidden: You do not have permission to access this resource.");
        case 404:
          throw new Error(data.message || "Not Found: The requested resource was not found.");
        case 500:
