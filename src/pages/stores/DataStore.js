...
const useDataStore = create(
  persist(
    (set, get) => ({
      ...
      // Encompasses getModels, getModelRuns
      getPipeline: async (projectName, accessToken) => {
        // Get models first
        await get().getModels(projectName, null, accessToken);

        // Get all project runs' handoffs
        const projectRuns = get().projectRuns;
        const aggregatedHandoffs = [];

        if (projectRuns && projectRuns.length > 0) {
          for (const projectRun of projectRuns) {
            if (projectRun && projectRun.name) {
              await get().getHandoffs(
                projectName,
                projectRun.name,
                accessToken,
              );
              // After each getHandoffs call, aggregate the new handoffs
              const currentHandoffs = get().handoffs;
              aggregatedHandoffs.push(...currentHandoffs);
            }
          }
        }

        // Set the final aggregated handoffs
        console.log(aggregatedHandoffs);
        set({ handoffs: aggregatedHandoffs });
      },
